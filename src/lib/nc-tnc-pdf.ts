...
function drawHeader(doc: jsPDF, logo: string | null, _numero: string) {
  ...
  doc.rect(6, y, PAGE_W - 12, h);
  doc.line(36, y, 36, y + h);
  doc.line(152, y, 152, y + h);

  if (logo) {
    try { doc.addImage(logo, "PNG", 8, y + 1.5, 26, 11, undefined, "FAST"); } catch {}
  } else {
    doc.setFont("helvetica", "bold"); doc.setFontSize(10);
    doc.text("DMN", 21, y + 8, { align: "center" });
  }

  doc.setFont("helvetica", "bold"); doc.setFontSize(12);
  doc.setTextColor(0);
  doc.text("Tratativa de Não Conformidade - TNC", 94, y + 8.5, { align: "center" });

  doc.setFont("helvetica", "normal"); doc.setFontSize(7);
  const mx = 154;
  doc.text("CÓD.: FORCP-SGI-05", mx, y + 3);
  doc.text("REVISÃO: 00", mx, y + 6.5);
  doc.text("DATA: 28/05/2025", mx, y + 10);
  doc.text("PÁG.: 1/1", mx, y + 13.5);
}
...
function drawSideBand(doc: jsPDF, yStart: number, yEnd: number, label: string) {
  ...
  // Calcular tamanho de fonte que caiba na altura disponível
  const h = yEnd - yStart;
  const approxCharW = 1.7; // mm por char @ size 8
  const maxChars = Math.max(8, Math.floor((h - 2) / approxCharW));
  const fontSize = label.length > maxChars ? Math.max(5, Math.floor((h - 2) / (label.length * 0.55))) : 8;
  doc.setFontSize(fontSize);
  const cy = (yStart + yEnd) / 2;
  doc.text(label, SIDE_X + SIDE_W / 2 + 1, cy, { angle: 90, align: "center" });
}
...
function sectionBar(doc: jsPDF, y: number, text: string, x = CONTENT_X, w = CONTENT_W): number {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(text, x + w / 2, y + 3.1, { align: "center" });
  return y + h;
}
...
export async function generateTNCPdf(nc: NCData): Promise<jsPDF> {
  ...
  autoTable(doc, {
    ...
    styles: { ...TIGHT, minCellHeight: 12, cellPadding: 1.4 },
    body: [[nc.descricao ?? ""]],
  });
  y = (doc as any).lastAutoTable.finalY;
  ...
  autoTable(doc, {
    startY: y, margin: { left: CONTENT_X, right: PAGE_W - CONTENT_R }, theme: "grid",
    styles: { ...TIGHT, minCellHeight: 9, cellPadding: 1.4 },
    body: [[nc.abrangencia ?? ""]],
  });
  y = (doc as any).lastAutoTable.finalY;
  ...
  autoTable(doc, {
    startY: y, margin: { left: CONTENT_X, right: PAGE_W - CONTENT_R }, theme: "grid",
    head: [[
      { content: "4- AÇÕES IMEDIATAS", styles: { halign: "center", fillColor: C_HEADER, textColor: 0, fontStyle: "bold", fontSize: 8 } } as any,
      { content: "RESPONSÁVEL", styles: { halign: "center", fillColor: C_HEADER, textColor: 0, fontStyle: "bold", fontSize: 8 } } as any,
      { content: "PRAZO", styles: { halign: "center", fillColor: C_HEADER, textColor: 0, fontStyle: "bold", fontSize: 8 } } as any,
    ]],
  ...
  autoTable(doc, {
    startY: y, margin: { left: CONTENT_X, right: PAGE_W - CONTENT_R }, theme: "grid",
    head: [
      [{ content: "6- AÇÕES CORRETIVAS", colSpan: 3, styles: { halign: "center", fillColor: C_HEADER, textColor: 0, fontStyle: "bold", fontSize: 8 } } as any],
      [
        { content: "AÇÕES", styles: { halign: "center", fillColor: C_HEADER, textColor: 0, fontStyle: "bold", fontSize: 7.5 } } as any,
  ...
  doc.text("DMN Estaleiro · FORCP-SGI-05 · Revisão 00 · 28/05/2025", 6, pageH - 4);
