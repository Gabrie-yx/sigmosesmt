...
export function PteLookupSheet({
  ...
  const { data: ptes = [] } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("ptes")
          .select("id,numero,data,data_emissao,risco,local,status,employee_name,apr_id")
  ...
  const { data: emps = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("employees")
        .select("id,nome,matricula,company_id,status,role_id")
        .eq("status", "ATIVO")
        .order("nome");
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((p) =>
      `${p.numero ?? ""} ${p.risco ?? ""} ${p.local ?? ""} ${p.employee_name ?? ""}`
        .toLowerCase()
        .includes(q),
    );
  ...
  return createPortal(
    <div
      className="pointer-events-auto fixed inset-0 z-[120] flex justify-end"
      role="dialog"
      aria-modal="true"
      aria-label="Vincular PTE à APR"
    >
      <button
        type="button"
        ...
        <div className="relative z-10 flex h-full min-h-0 flex-col">
          <header className="px-5 py-4 border-b border-rose-900/50 shrink-0">
            ...
            <p className="mt-1 text-xs text-rose-200/70">
              {aprNumero ? (
                <>
                  APR <b className="text-rose-100">{aprNumero}</b>
                </>
              ) : (
                "Nova APR (não salva)"
              )}{" "}
          ...
          >
            <TabsList className="mx-5 mt-3 grid grid-cols-2 shrink-0">
              <TabsTrigger value="buscar">
                <Search className="h-3.5 w-3.5 mr-1" /> Buscar existente
              </TabsTrigger>
              <TabsTrigger value="nova">
                <Plus className="h-3.5 w-3.5 mr-1" /> Nova PTE
              </TabsTrigger>
            </TabsList>

            <TabsContent
              value="buscar"
              className="flex-1 overflow-hidden flex flex-col p-5 pt-3 m-0"
            >
              {riscoSugerido && (
              ...
              <Input
                ...
                className="mb-3 shrink-0 border-rose-900/60 bg-black/35 text-rose-50 placeholder:text-rose-200/40"
              />
                ...
                {filtered.length === 0 ? (
                  <div className="text-center text-xs text-rose-200/60 py-8">
                    ...
                    {filtrarPorRisco && riscoSugerido && (
                      <div className="mt-1 text-[10px]">
                        Tente "Mostrar todas" ou crie uma nova na aba ao lado.
                      </div>
                    )}
                  </div>
                ) : (
                  filtered.map((p: any) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`border rounded-lg p-3 transition ${mismatch ? "border-rose-500/50 bg-rose-950/35 hover:border-rose-400" : "border-rose-900/45 bg-black/30 hover:border-rose-600/70 hover:bg-rose-950/25"}`}
                      >
                        <div className="flex items-center justify-between gap-2 mb-1">
                          ...
                          <span
                            className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${p.status === "ATIVA" ? "bg-emerald-500/15 text-emerald-200 border border-emerald-400/25" : "bg-white/10 text-rose-200"}`}
                          >
                            {p.status}
                          </span>
                        </div>
                        <div className="text-[11px] text-rose-100/85">
                          <b>Risco:</b> {p.risco ?? "—"}
                        </div>
                        <div className="text-[11px] text-rose-100/85">
                          <b>Local:</b> {p.local ?? "—"}
                        ...
                        {p.employee_name && (
                          <div className="text-[11px] text-rose-200/70">
                            <b>Executante:</b> {p.employee_name}
                          </div>
                        )}
                        <div className="text-[10px] text-rose-200/45 mt-1">
                          Emitida em {formatDateBR(p.data_emissao || p.data)}
                        </div>
                        {p.apr_id && p.apr_id !== aprId && (
                          <div className="text-[10px] text-rose-200/75 mt-0.5">
                            ⚠ Já vinculada a outra APR — vínculo será atualizado
                          </div>
                        )}
                        {mismatch && (
                          <div className="text-[10px] text-rose-100 mt-0.5 flex items-center gap-1">
                            <AlertTriangle className="h-3 w-3" /> Não cobre a categoria pendente "
                            {riscoSugerido}"
                          </div>
                        )}
                        <Button
                          size="sm"
                          className="w-full mt-2 h-8 bg-gradient-to-br from-rose-700 to-rose-950 text-xs text-rose-50 hover:from-rose-600 hover:to-rose-900"
            ...
            <TabsContent value="nova" className="flex-1 overflow-y-auto p-5 pt-3 m-0 space-y-3">
              <div>
                ...
                <Input
                  ...
                  className="mt-1 border-rose-900/60 bg-black/35 text-rose-50"
                />
              </div>
              <div>
                ...
                <Select value={form.risco} onValueChange={(v) => setForm({ ...form, risco: v })}>
                  <SelectTrigger className="mt-1 border-rose-900/60 bg-black/35 text-rose-50">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {PTE_RISCOS.map((r) => (
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({ ...form, company_id: v === "none" ? "" : v, employee_id: "" })
                  }
                >
                  <SelectTrigger className="mt-1 border-rose-900/60 bg-black/35 text-rose-50">
              ...
              <div>
                <Label className="text-[10px] font-black uppercase text-rose-200/70">
                  Executante{" "}
                  {selectedCompanyId ? "(filtrado pela empresa)" : "(selecione uma empresa)"}
                </Label>
                <Select
                ...
                >
                  <SelectTrigger className="mt-1 border-rose-900/60 bg-black/35 text-rose-50">
                    <SelectValue
                      placeholder={
                        selectedCompanyId ? "Selecionar..." : "Escolha a empresa primeiro"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {!aprId && (
                <div className="text-[10px] bg-rose-950/35 border border-rose-800/50 rounded px-2 py-1.5 text-rose-100/80">
                  A APR ainda não foi salva — a nova PTE será criada sem vínculo. Salve a APR e use
                  "Buscar existente" para vincular depois.
                </div>
              )}
