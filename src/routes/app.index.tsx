    ...
    <div className="h-full overflow-y-auto custom-scrollbar bg-background text-foreground">
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 md:px-14">
        ...
        <div className="mt-10 grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="space-y-6">
            {["Prioriza o dia", "Evita vencimentos", "Distribui responsabilidades"].map((title, index) => <div key={title} className="flex gap-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-primary/30 bg-primary/10 text-base font-black text-primary">0{index + 1}</div><div><h3 className="text-base font-bold">{title}</h3><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{index === 0 ? "Reúne as tarefas críticas para começar o expediente sabendo onde agir." : index === 1 ? "Antecipa ASOs, treinamentos, documentos e inspeções antes do prazo." : "Cada registro guarda responsável, andamento, evidência e conclusão."}</p></div></div>)}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 md:px-14">
        <div className="grid items-center gap-10 lg:grid-cols-[0.82fr_1.18fr]">
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-card shadow-xl">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3"><div className="flex items-center gap-2 text-sm font-black"><GraduationCap className="h-5 w-5 text-primary" /> Matriz de Treinamento</div><div className="flex gap-2"><span className="rounded-sm border border-border px-2 py-1 text-xs text-muted-foreground">Empresa</span><span className="rounded-sm border border-border px-2 py-1 text-xs text-muted-foreground">Situação</span></div></div>
            <div className="overflow-x-auto p-4"><div className="min-w-[520px]"><div className="grid grid-cols-[1.4fr_repeat(5,1fr)] gap-1 text-center text-xs font-bold text-muted-foreground"><div className="text-left">FUNÇÃO</div>{["NR-01", "NR-06", "NR-10", "NR-33", "NR-35"].map((nr) => <div key={nr}>{nr}</div>)}</div>{["Técnico de segurança", "Eletricista", "Operador", "Supervisor"].map((role, row) => <div key={role} className="mt-1 grid grid-cols-[1.4fr_repeat(5,1fr)] gap-1"><div className="flex min-h-9 items-center rounded-sm bg-muted/50 px-2 text-xs font-semibold">{role}</div>{Array.from({ length: 5 }, (_, col) => { const state = (row + col) % 4; return <div key={col} className={`min-h-9 rounded-sm ${state === 0 ? "bg-primary/80" : state === 1 ? "bg-chart-3/70" : state === 2 ? "bg-destructive/70" : "bg-chart-2/70"}`} />; })}</div>)}</div></div>
          </div>
        </div>
      </section>
